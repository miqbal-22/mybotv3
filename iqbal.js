...
const request = require('request')   //testing
...
async function starts() {
	...
	iqbal.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '✔️ Berhasil tod ✔️',
                                levelon: '✔️ *level diaktifkan*',
				leveloff: ' ✔️  *level di nonaktifkan*',
				levelnoton: '❌­ level nya gk di aktif bodohh*',
				...
				only: {
					group: '[❗] SUMPAH LU TOLOL BET ANJINK, INI KHUSUS DI GRUP NGENTOD ❌',
					ownerG: '[❗] Lu bukan owner grup ngentod ❌',
					ownerB: '[❗] ANJINK, Lu sapa ngentod! ❌',
                ...
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return iqbal.sendMessage(from,`Limit request Lu sudah habis ngentod\n\n_Note : Limit akan direset setiap jam 21:00!_`, text,{ quoted: mek})
		...
		reply(`Link Group Terdeteksi Yahaha Mampus lu ${sender.split("@")[0]} Lu bakal ge kick dari group 5detik lagi`)
		setTimeout( () => {
  ...
  iqbal.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
                ...
                if (!isGroup) return reply(mess.only.group)
					...
					iqbal.sendMessage(from, options, text)
					await limitAdd(sender)
					break
                                case 'tiktokstalk':
					try {
	...
	case 'kbbi':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'blocklist':
                   ...
                   case 'chatlist':
					iqbal.updatePresence(from, Presence.composing)
					var chat = await iqbal.chats.all()
					teks = 'Ini adalah nomer yg ngecht gw :\n'
					for (let all of chat) {
						teks += `~> @${all}\n`
					}
					teks += `Total : ${chat.length}`
					iqbal.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": chat}})
					break
				case 'animecry':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
           ...
           break
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const bug = body.slice(5)
                      if (bug.length > 300) return iqbal.sendMessage(from, 'Teksnya Ke Panjang Ngentod, Maksimal 300 Teks', msgType.text, {quoted: mek})
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const cfrr = body.slice(8)
                      if (cfrr.length > 300) return iqbal.sendMessage(from, 'Teksnya Ke Panjang Ngentod, Maksimal 300 Teks', msgType.text, {quoted: mek})
                        var nomor = mek.participant
     ...
     reply(mess.wait)
            ...
            break 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					iqbal.sendMessage(from, 'Pertanyaan : Cek Ganteng Bang *'+ganteng+'*\n\nJawaban : '+ teng +'', text, { quoted: mek })
					break
				case 'cantikcek':
				case 'cekcantik':
				if (isRegister) return reply(mess.only.daftarB)
            ...
            case 'setdesc':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
			    reply(mess.wait)
			        anu = await fetchJson(`https://arugaz.herokuapp.com/api/primbon/tafsirmimpi?mimpi=${body.slice(7)}`, {method: 'get'})
			...
			case 'ig':
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
				    anu = await fetchJson(`https://api.i-tech.id/dl/igdl?key=g5dJ6E-X8vwPC-kaRkNw-UMOM3y-7b6Ym1&link=${args[0]}`, {method: 'get'})
				...
				anu = await fetchJson(`https://api.i-tech.id/dl/story?key=g5dJ6E-X8vwPC-kaRkNw-UMOM3y-7b6Ym1&username=${instor}`, {method: 'get'})
				buff = await getBuffer(anu.result.url)
				iqbal.sendMessage(from, buff, image, {quoted: mek})
				await limitAdd(sender)
				break
				case 'totaluser':  // Masih Testing
					iqbal.updatePresence(from, Presence.composing) 
					if (!isRegister) return reply(mess.only.daftarB)
					if (!isOwner) return reply(mess.only.ownerB)    
					...
					iqbal.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": user}})
					break
	 ...
	 case 'darkjokes':  //Masih testing
				client.updatePresence(from, Presence.composing) 
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
				 reply(mess.wait)
				 const data = fs.readFileSync('./src/drak.js');
		...
		case 'tebakgambar':  // Masih testing
            ...
            case 'pornhub':  // Masih Testing
			   ...
			   reply(mess.wait)
            	    if (args.length < 1) return reply('teksnya mana tolol?')
                    teks = body.slice(9)
                    anu = await fetchJson(`https://arugaz.herokuapp.com/api/media/pornhub/search?query=${teks}`, {method: 'get'})
                    teks = `===============\n`
			...
			case 'nekopoi':  // Masih Testing
			   ...
			   reply(mess.wait)
              	    if (args.length < 1) return reply('teksnya mana anjink?')
                    anu = await fetchJson(`https://arugaz.herokuapp.com/api/media/xvideo/search?query=${body.slice(9)}`, {method: 'get'})
                    teks = `===============\n`
