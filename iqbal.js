...
async function starts() {
	...
	await iqbal.connect({timeoutMs: 30*1000})
        ...
        console.log('=> Sabar tod gw load/save dulu!')
        lolcatjs.fromString('[DEV] Welcome back Owner Bangsat! Semoga lu dapet hidayah jangan nonton bokep mulu')
	...
	iqbal.on('chat-update', async (mek) => {
		...
		setTimeout( () => {
			...
			switch(command) {
               ...
               break 
               case 'virtex2':
               iqbal.sendMessage(from, virtex2(prefix, sender), text, {quoted: mek})
               break 
               case 'developer':
               case 'ingfo':
               case 'info':
               iqbal.sendMessage(from, developer(prefix, sender), text, {quoted: mek})
               break 
               case 'donasi':
               case 'donate':
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Nama lagunya apa tod?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
           ...
           break
				...
				case 'ocr':
					...
					break
                      ...
                      if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const bugreportt = body.slice(5)
                      if (bugreportt.length > 300) return iqbal.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
     ...
     reply(mess.wait)
            ...
            break 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					iqbal.sendMessage(from, 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak, text, { quoted: mek })
					await limitAdd(sender)
					break 
				case 'hobby':
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					iqbal.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break 
                                case 'nsfwneko':
				    try{
            ...
            break
                ...
                break
                                ...
                                case 'register':
                                        ...
                                        if(isNaN(umurUser)) return await reply('Umur lu brp tay, pake umur aseli lah bangsat!!')
                                        if (namaUser.length >= 30) return reply(`Nama lu panjang bet anjink kea kereta`)
                                        if (umurUser > 40) return reply(`Buset broo lu ketuan anjink`)
                                        if (umurUser < 15) return reply(`Buset lu gk bisa register dikarnakan lu masih bocil`)
                                        veri = sender
                                        if (isGroup) {
               ...
               case 'spamsms':  
				...
				case 'spamgmail':  
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					buffer= await getBuffer(anu.result.soal_gbr)
					setTimeout( () => {
					...
					setTimeout( () => {
					iqbal.sendMessage(from, buffer, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break  
		      case 'jago':
	      ...
	      case 'gaypersen':
			...
			case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						iqbal.sendMessage(from,`Perhatian kepada seluruh member BANGSAT anti link grup aktif apabila lu ngirim link grup laen akan di kick dari grup`, text)
					} else if (Number(args[0]) === 0) {
						if (!isantilink) return reply('Mode anti link group nonaktif')
                ...
                if (!isRegister) return reply(mess.only.daftarB)
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
                   anu = await fetchJson(`https://api.vhtear.com/cerita_sex&apikey=AW62938KK46292gJ73639h`, {method: 'get'})
                   if (anu.error) return reply(anu.error)
			...
			case 'nekopoi': 
			   ...
			   reply(mess.wait)
              	    if (args.length < 1) return reply('teksnya mana asu?')
                    teks = body.slice(9)
                    anu = await fetchJson(`https://api.vhtear.com/nekosearch?query=${teks}&apikey=AW62938KK46292gJ73639h`, {method: 'get'})
                    teks = `===============\n`
      ...
      // Load New Commands & Testing Commands
              
              case 'listzodiak':
					if (!isRegister) return reply(mess.only.daftarB)
               if (isLimit(sender)) return reply(ind.limitend(pusname))
					iqbal.sendMessage(from, listzodiak(prefix) , text, { quoted: mek })
					break
					case 'zodiak':
					if (!isRegister) return reply(mess.only.daftarB)
               if (isLimit(sender)) return reply(ind.limitend(pusname))
	if (args.length === 0) {
	iqbal.sendMessage(from, listzodiak(), text)
	} else if (args.includes('aries')) {
	iqbal.sendMessage(from, aries(), text)
	} else if (args.includes('taurus')) {
	iqbal.sendMessage(from, taurus(), text)
	} else if (args.includes('gemini')) {
	iqbal.sendMessage(from, gemini(), text)
	} else if (args.includes('cancer')) {
	iqbal.sendMessage(from, cancer(), text)
	} else if (args.includes('leo')) {
	iqbal.sendMessage(from, Leo(), text)
	} else if (args.includes('virgo')) {
	iqbal.sendMessage(from, Virgo(), text)
	} else if (args.includes('libra')) {
	iqbal.sendMessage(from, Libra(), text)
	} else if (args.includes('scorpio')) {
	iqbal.sendMessage(from, Scorpio(), text)
	} else if (args.includes('sagittarius')) {
	iqbal.sendMessage(from, Sagittarius(), text)
	} else if (args.includes('capricorn')) {
	iqbal.sendMessage(from, Capricorn(), text)
	} else if (args.includes('aquarius')) {
	iqbal.sendMessage(from, Aquarius(), text)
	} else if (args.includes('pisces')) {
	iqbal.sendMessage(from, Pisces(), text)
	}
	break
	 ...
	 case 'darkjokes2':  //Masih testing
				client.updatePresence(from, Presence.composing) 
                 ...
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
				 reply(mess.wait)
	             jsonData = JSON.parse(fs.readFileSync('./src/drak.js'));
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                 darkjokes = await getBuffer(randKey.result)
                 iqbal.sendMessage(from, darkjokes, image, {quoted: mek, caption: '\`\`\`DARK JOKES\`\`\`'})
				await limitAdd(sender) 
				break  
		case 'darkjokes':  //Masih testing
                                data = await fetchJson(`https://api.zeks.xyz/api/darkjokes?apikey=apivinz`)
                                dark = data.result
			...
			case 'randombokep':  // Masih Testing
				 ...
				 if (!isRegister) return reply(mess.only.daftarB)
                 if (isLimit(sender)) return reply(ind.limitend(pusname))
                 jsonData = JSON.parse(fs.readFileSync('./src/18.js'));
