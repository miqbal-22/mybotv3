	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo Ngentoters @${num.split('@')[0]}\nSelamat datang di group *${mdata.subject}*`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '[❗] Bacot lu lu itu cuma member yaa ngentod jadi jangan sok keras yaa anjink, Perintah ini hanya bisa di gunakan oleh admin group! ❌',
					Badmin: '[❗] Jadiin gw admin dulu ngentod, Perintah ini hanya bisa di gunakan ketika bot menjadi admin! ❌',
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin di promote ngentod!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
               ...
               client.sendMessage(from, 'Nih nomor owner gw tod, save ya tod nanti di save balik',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Usernamenya mana ngentod? ', text, {quoted: mek})
                                                if (!isUser) return reply(mess.only.daftarB)
						...
						reply('username gk valid ngentod')
					}
					break
				case 'snow':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'ytmp3':
					...
					teks = `❏ *Title* : ${anu.title}\n❏ *Filesize* : ${anu.filesize}\n\nTunggu Bentar Ya Ngentod, Audionya Lagi Di Kirim...`
				...
				case 'testime':
					...
					break
				case 'semoji':
					if (args.length < 1) return reply('emojinya mana ngentod?')
                                        if (!isUser) return reply(mess.only.daftarB)
					ranp = getRandom('.png')
					rano = getRandom('.webp')
				...
				case 'nulis': 
				case 'tulis':
					if (args.length < 1) return reply('Gw suruh nulis apa ngentod?')
                                        if (!isUser) return reply(mess.only.daftarB)
					teks = body.slice(7)
					reply(mess.wait)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Kode bahasanya mana Ngentod?', text, {quoted: mek})
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							if (err) return reply('Gagal ngentod:(')
				...
				if (!isQuotedImage) return reply(`Kirim gambar dengan caption ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					...
					reply('Makasih tod profil barunya😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'alay':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Jingan👋', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('judul lagunya mana anjink')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Lirik lagunya mana ngentod?')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Gunakan kode negara nya ngentod')
					try {
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'Yahaha Lu kekick mampus anjink😂', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "Command Lu Salah Ngentod", text, { quoted: mek })
		           if (stdout) {
				...
				case 'ocr':
                    ...
                    reply('Masalah telah di laporkan ke owner BOT, laporan palsu/main2 tidak akan ditanggapi yaa ngentod.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Gagal anjink, pada saat pengubahan ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('❌ reply stickernya ngentod ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Gagal tod, pada saat pengunahan sticker ke gambar ❌')
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					break
                                  case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Lu udah terdaftar ngentod')
				...
				if (isUser) return reply(mess.only.daftarB)
					hobby = body.slice(1)
					const hob =['Memasak','Ngentotin emak sendiri','Ngentotin anak orang','Mabar','Nobar','Sosmedtan''Ngentotin pembantu','Nge gay/Lesbi','Membantu orang lain','Naik mobil','Ngewe','Nonton anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Ngocok peler/colmek','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri','Coli']
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf Iqbal|Ganss`)
					reply(mess.wait)
                ...
                case 'truth':
					...
					break
                                case 'dare':
					const dare =['Kirim pesan ke mantan kamu dan bilang "aku masih suka sama kamu','telfon crush/pacar sekarang dan ss ke pemain','pap ke salah satu anggota grup','Bilang "KAMU CANTIK BANGET NGGAK BOHONG" ke cowo','ss recent call whatsapp','drop emot "ðŸ¦„ðŸ’¨" setiap ngetik di gc/pc selama 1 hari','kirim voice note bilang can i call u baby?','drop kutipan lagu/quote, terus tag member yang cocok buat kutipan itu','pake foto sule sampe 3 hari','ketik pake bahasa daerah 24 jam','ganti nama menjadi "gue anak lucinta luna" selama 5 jam','chat ke kontak wa urutan sesuai %batre kamu, terus bilang ke dia "i lucky to hv you','prank chat mantan dan bilang " i love u, pgn balikan','record voice baca surah al-kafirun','bilang "i hv crush on you, mau jadi pacarku gak?" ke lawan jenis yang terakhir bgt kamu chat (serah di wa/tele/ig/fb/dll), tunggu dia bales, kalo udah ss drop ke sini','sebutkan tipe pacar mu!','snap/post foto pacar/crush','teriak gajelas lalu kirim pake vn kesini','pap mukamu lalu kirim ke salah satu temanmu','kirim fotomu dengan caption, aku anak pungut','teriak pake kata kasar sambil vn trus kirim kesini','teriak " annnjiiinggg gabutt ngentooddd " di depan rumah mu','ganti nama jadi " BOWO " selama 24 jam','Pura pura kerasukan, contoh : kerasukan maung, kerasukan belalang, kerasukan kulkas, dll']
            ...
            break
                                ...
                                case 'neonlogo':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih anjink logonya...'})
                                        break
                                case 'neonlogo2':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih anjing logonya...'})
                                        break
                                case 'lionlogo':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                ...
                                /*case 'jsholat':
                                        tels = body.slice(8)
                                        if (args.length < 1) return reply('Daerahnya dimana ngentod?gk punya rumah lu?')
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *SEDANG AFK ${tels} JANGAN GANGGU NGENTOD*`,
                                ...
                                case 'shadow':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih tod gambarnya...'})
                                        break
                                case 'burnpaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                ...
                                case 'mutgrass':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih gambarnya Asu...'})
                                        break
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                ...
                                case 'randomkpop':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih kpopnya Anjink....'})
                                        break
                                case 'fml2':
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('Ini siapa?')
					anu = await client.chats.all()
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Textnya mana ngentod?')
					teks = body.slice(5)
				...
				case 'clone':
					...
					break
                                /*case 'magernulis':
                                        ...
                                        if (args.length < 1) return reply('teks, nama, kelas nya mana tod?\nContoh: ${prefix}magernulis teks|nama|kelas\nContoh pemakaian: ${prefix}magernulis IqbalGans|Iqbal|XI TM2/11Tm2, sama ajjh anjink')
                                        if (!isUser) return reply(mess.only.daftarB)
                                ...
                                case 'pubglogo':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih logonya yaa bangsat...'})
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
