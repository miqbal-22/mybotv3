			...
			mess = {
				...
				only: {
					...
					Badmin: '[❗] Jadiin gw admin dulu ngentod, Perintah ini hanya bisa di gunakan ketika bot menjadi admin! ❌',
                                        daftarB: `──「 DAFTAR 」──\nHallo Ngentoters !\nLu belum Terdaftar didalam database, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar iqbal|666`,
				}
			}
               ...
               break
				case 'demote':
                  ...
                  case 'promote':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag target yang ingin lu promote tod!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				if (!isQuotedImage) return reply(`Kirim gambar dengan caption ${prefix}setbotpp atau tag gambar yang sudah dikirim`)
					...
					reply('Makasih profil barunya tod😗')
					break
				case 'bc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'chord':
					if (args.length < 1) return reply('judul lagunya mana ngentod')
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)					
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
              ...
              case 'bolehkah':
                client.updatePresence(from, Presence.composing) 
              if (!isUser) return reply(mess.only.daftarB)
                random = bolehka[Math.floor(Math.random() * (bisakah.length))]
				...
				case 'clone':
					...
					break
                                ...
                                case 'translate':
                                        ...
                                        if (args.length < 1) return reply(`kode bahasanya mana tod\nContoh:\n${prefix}translate|en|gua iqbal`)
                                        if (args.length < 2) return reply(`teksnya mana tod\nContoh:\n${prefix}translate|en|gua iqbal`)
                                        if (!isUser) return reply(mess.only.daftarB)
                                        reply(mess.wait)
