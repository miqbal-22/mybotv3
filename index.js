	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const antilink = anlink.includes(sender)
                        ...
                        const BarBarKey = 'WEMPY240905'
                        const ZeksApi = 'apivinz'
                        const TobzApi = 'BotWeA'
                        const TechApi = 'g5dJ6E-X8vwPC-kaRkNw-UMOM3y-7b6Ym1'
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                const check = await mek.inviteInfo(iqbal);
                if (!check) {
                    return
                } else {
                    reply('*[GROUP LINK DETECTOR!]*\nLu ngirim link grup chat kan?, mampus lu segera di kick dari grup.').then(() => {
                        client.groupRemove()
                    })
                }
            }
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
               case 'info':
               client.sendMessage(from, info(prefix, sender), text, {quoted: mek})
               break
               case 'kodenegara':
               ...
               client.updatePresence(from, Presence.composing) 
				...
				case 'wikien':
					...
					tels = body.slice(8)		
			                if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.arugaz.my.id/api/edu/enwiki?query=${tels}`, {method: 'get'})
					reply(anu.result)
                                        await limitAdd(sender)
					break				
				case 'ytmp3':
				...
				reply(mess.wait)
					...
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek, caption: 'Nih tod'})
					await limitAdd(sender) 
					break 
	...
	case 'kbbi':
			  ...
			  case 'totaluser':
				...
				case 'send':
					...
					case 'quotesnime':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					nimek = await fetchJson('https://api.arugaz.my.id/api/anime/animequotes')
				...
				case 'alay':
                    ...
                    if (isLimit(sender)) return reply(ind.limitend(pusname))
                    data = await fetchJson(`https://api.arugaz.my.id/api/edu/corona?country=indonesia}`)
                    reply(data.result)
                    await limitAdd(sender)
                    break
                    case 'quotemaker':
					...
					reply(ind.wait())
					aruga = await getBuffer(`https://api.arugaz.my.id/api/textpro/avengers?text1=${aruga1}&text2=${aruga2}`)
				...
				case 'chord':
					if (args.length < 1) return reply('judul lagunya mana kak')
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
					anu = await fetchJson(`https://api.arugaz.my.id/api/chord?q=${tels}`, {method: 'get'})
					reply(anu.result)
                                        await limitAdd(sender)
					break
				case 'lirik':
					if (args.length < 1) return reply('Nama lagunya apa kak?')
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					anu = await fetchJson(`https://api.arugaz.my.id/api/lirik?judul=${tels}`, {method: 'get'})
					reply(anu.result)
                                        await limitAdd(sender)
					break
			case 'igstalk':
           ...
           break
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const pesan = body.slice(8)
                      if (pesan.length > 300) return client.sendMessage(from, 'Teksnya Ke Panjang Ngentod, Maksimal 300 Teks', text, {quoted: mek})
                    ...
                    client.sendMessage('6285745376798@s.whatsapp.net', options, text, {quoted: mek})
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                     const cfrr = body.slice(8)
                      if (cfrr.length > 300) return client.sendMessage(from, 'Teksnya Ke Panjang Ngentod, Maksimal 300 Teks', text, {quoted: mek})
                        var nomor = mek.participant
     ...
     reply(mess.wait)
            ...
            break 
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('fitur antilink udah aktif tod')
						anlink.push(from)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(anlink))
						reply('❬ SUCCSESS ❭ mengaktifkan fitur antilink di group ini')
					} else if (Number(args[0]) === 0) {
						anlink.splice(from, disable)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(anlink))
					...
					if (isOwnerBot) {
					  ...
					  if (cgrup.length > 20) return client.reply(from, `Sorry bro grup di bot ini sudah full `, id)
					  if (cgrup.size < 50) return client.reply(from, `Huek Cuihh, member cuma ${memberLimit} orang mau invite bot, klo mau invite bot minimal 50 member`, id)
					  await client.joinGroupViaLink(linkgrup)
					    .then(async () => {
			    ...
			    case 'waifu':
					...
					reply(mess.wait)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://api.arugaz.my.id/api/nekonime`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image,{quoted: mek})
                                        await limitAdd(sender)
					break
				...
				case 'clone':
					...
					break
                                ...
                                case 'fancytext':
                                        ...
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://api.arugaz.my.id/api/random/text/fancytext?text=${teks1}`, {method: 'get'})
                                        reply(anu.result)
                                        await limitAdd(sender)
                                        break
                                case 'bal':
            ...
            case 'setdesc':
                ...
                break
                                case 'translate':
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://api.arugaz.my.id/api/edu/translate?lang=${lang}&text=${teksnya}`, {method: 'get'})
              ...
              case 'spamcall':  
                    ...
                    data = await fetchJson(`https://api.arugaz.my.id/api/spamcall?no=${body.slice(10)}`)
               ...
               case 'spamsms':  
					...
					tels = body.slice(9)
					reply(mess.wait)
					anu = await fetchJson(`https://api.arugaz.my.id/api/spamsms?no=${tels}&jum=10`, {method: 'get'})
					reply(anu.logs)
					break	
				case 'spamgmail':  
					...
					tels = body.slice(11)
					reply(mess.wait)
					anu = await fetchJson(`https://api.arugaz.my.id/api/spamgmail?target=${tels}&jum=10`, {method: 'get'})
					reply(anu.logs)
					break	
	   case 'darkjokes':
				client.updatePresence(from, Presence.composing) 
